...
void CCamera::render()
{
	...
	// 하기전 TexParam삭제.
	// 
	//static Ptr<CMaterial>mtrl = CResMgr::GetInst()->FindRes<CMaterial>(L"Std3D_DeferredMtrl");
	//mtrl->SetTexParam(TEX_0, nullptr);
	//mtrl->SetTexParam(TEX_3, nullptr);
	...
	for (UINT i = 0; i < vecLight3D.size(); ++i)
	{
		vecLight3D[i]->render();
	}
	...
	static Ptr<CMaterial> pMaterial = CResMgr::GetInst()->FindRes<CMaterial>(L"MergeMtrl");

	static bool bSet = false;
	if (!bSet)
	{
		bSet = true;
		pMaterial->SetTexParam(TEX_0, CResMgr::GetInst()->FindRes<CTexture>(L"ColorTargetTex"));
		pMaterial->SetTexParam(TEX_1, CResMgr::GetInst()->FindRes<CTexture>(L"DiffuseTargetTex"));
		pMaterial->SetTexParam(TEX_2, CResMgr::GetInst()->FindRes<CTexture>(L"SpecularTargetTex"));
		pMaterial->SetTexParam(TEX_3, CResMgr::GetInst()->FindRes<CTexture>(L"EmissiveTargetTex"));
	}
	pMaterial->UpdateData();
	pRectMesh->render();
