...
void CalcLight3D(float3 _vViewPos, float3 _vViewNormal, int _LightIdx, inout tLightColor _Light, inout float _SpecPow)
{
   ...
   // DirLight 인 경우
    ...
    else if (1 == LightInfo.LightType)
    {
        ...
        // 광원과 빛을 받는 위치 사이의 거리에 따라서 fDisPow가 달라짐.
        float fDist = distance(_vViewPos, vViewLightPos);
        fDistPow = saturate(cos((fDist / LightInfo.Radius) * (PI / 2.f)));
        
        // ViewSpace 에서의 노말벡터와 광원의 방향을 내적 후 fDistPow 곱해주기.
        fLightPow = saturate(dot(_vViewNormal, -vViewLightDir)) * fDistPow;

        // 반사광
        float3 vViewReflect = normalize(2.f * _vViewNormal * dot(-vViewLightDir, _vViewNormal) + vViewLightDir);
        float3 vEye = normalize(_vViewPos);
    ...
    else if (2 == LightInfo.LightType)
    {
        ...
        // 빛의 범위 안에 있는지 판별.
        float isInsideLight = 0.f;
        float3 vViewLightPos = mul(float4(LightInfo.vWorldPos.xyz, 1.f), g_matView).xyz;

        float3 vViewOriginLightDir = normalize(mul(float4(normalize(LightInfo.vWorldDir.xyz), 0.f), g_matView)).xyz;
        vViewLightDir = normalize(_vViewPos - vViewLightPos);

        if (dot(vViewOriginLightDir, vViewLightDir) <= cos(LightInfo.Angle / 2.f))
        {
            isInsideLight = 0.f;
        }
        else
        ...
        // ViewSpace 에서의 노말벡터와 광원의 방향을 내적 후 isInSideLight 곱해주기.
        fLightPow = saturate(dot(_vViewNormal, -vViewLightDir)) * isInsideLight;
        ...
        // 반사광의 세기 구하기
        fSpecPow = saturate(dot(vViewReflect, -vEye));
        fSpecPow = pow(fSpecPow, 20) * isInsideLight;
    }
