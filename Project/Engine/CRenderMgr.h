...
public:
    ...
    void RegisterLight2D(CLight2D* _Light2D, const tLightInfo& _Light2DInfo) { m_vecLight2D.push_back(_Light2D);  m_vecLight2DInfo.push_back(_Light2DInfo); }
    UINT RegisterLight3D(CLight3D* _Light3D, const tLightInfo& _Light3DInfo) 
    { 
        m_vecLight3D.push_back(_Light3D);  
        m_vecLight3DInfo.push_back(_Light3DInfo); 
